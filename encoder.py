...
class Encoder3D(Module):
    ...
    def __init__(self, cfg):
        ...
        if self.conv3d_max_pool:
            self.conv3d_pool_kernels = ast.literal_eval(
                self.args['conv3d_pool_kernels'])
            self.conv3d_pool_layers = [self.create_3d_pool_layer(
                n_layer) for n_layer in range(int(self.args['n_convs3d']))]
        else:
        ...
        if self.conv2d_max_pool:
            self.conv2d_pool_kernels = ast.literal_eval(
                self.args['conv2d_pool_kernels'])
            self.conv2d_pool_layers = [self.create_2d_pool_layer(
                n_layer) for n_layer in range(int(self.args['n_convs2d']))]
        else:
