...
import torch
from torch.nn import Dropout, Unflatten, Linear, Module, Sequential
from torch.nn import Conv3d, MaxPool3d, ConvTranspose3d, MaxUnpool3d
from torch.nn import Conv2d, MaxPool2d, ConvTranspose2d, MaxUnpool2d
...
class Decoder3D(Module):
    ...
    def __init__(self, cfg):
        ...
        if self.conv2d_max_unpool:
            self.conv2d_unpool_kernels = ast.literal_eval(self.args['conv2d_unpool_kernels'])
            self.conv2d_unpool_layers = [self.create_2d_unpool_layer(n_layer) for n_layer in range(int(self.args['n_convs2d']))]  
        else:
        ...
        if self.conv3d_max_unpool:
            self.conv3d_unpool_kernels = ast.literal_eval(self.args['conv3d_unpool_kernels'])
            self.conv3d_unpool_layers = [self.create_3d_unpool_layer(n_layer) for n_layer in range(int(self.args['n_convs3d']))]  
        else:
